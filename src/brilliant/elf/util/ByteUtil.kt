...
object ByteUtil {
    ...
    fun decHexSizeFormat32(size: ByteArray, isLittleEndian: Boolean): String {
        return ByteUtil.bytes2Int32(size, isLittleEndian).toString() + "(0x" + ByteUtil.bytes2Hex(size) + ")" + "B"
    }
    ...
    fun getStringFromMemory(index: Int, OS: OS): String {
        var index = index
        val sb = StringBuilder()

        while (true)
            if (OS.memory[index].toInt() != 0)
                sb.append(OS.memory[index++].toChar())
            else
                break
